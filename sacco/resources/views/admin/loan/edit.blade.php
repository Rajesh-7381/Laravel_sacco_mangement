    ...
    <section class="section">
      <div class="row">
        <div class="col-lg-12">
          ...
          <div class="card">
            <div class="card-body">
              <h5 class="card-title">Edit Loan Application</h5>
              

              <!-- General Form Elements -->
              <form action="{{url('admin/loan/edit/'.$getRecord->id)}}" enctype="multipart/form-data" method="post">
                {{ csrf_field() }}
                    ...
                    <div class="col-sm-10">
                        <select class="form-select" aria-label="Default select example" name="staff_id" required>
                            ...
                            @foreach($getStaff as $key => $value_4)
                                <option {{ $value_4->id == $getRecord->staff_id ? 'selected' : '' }} value="{{ $value_4->id }}">{{ $value_4->name }} {{ $value_4->last_name }} {{ $value_4->surename }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="col-sm-10">
                        <select class="form-select" aria-label="Default select example" name="loan_types_id" required>
                            ...
                            @foreach($getLoanTypes as $key => $value_11)
                                <option {{ $value_11->id == $getRecord->loan_types_id ? 'selected' : '' }} value="{{ $value_11->id }}">{{ $value_11->type_name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="col-sm-10">
                      <select class="form-select" aria-label="Default select example" name="loan_plan_id" required>
                        ...
                        @foreach($getLoanPlans as  $value_2)
                            <option {{ $value_2->id == $getRecord->loan_plan_id ? 'selected' : '' }} value="{{$value_2->id}}">{{$value_2->months}} [{{$value_2->intrest_percentage .'%'.  $value_2->penalty_rate }} ]</option>
                        @endforeach
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-sm-10">
                        <input 
                            ...
                            maxlength="10" required  value="{{$getRecord->loan_amount}}"
                        >
                    </div>
                </div>
                <div class="row mb-3">
                    ...
                    <div class="col-sm-10">
                        <input 
                            ...
                            name="purpose" 
                            id="purpose" 
                             required  value="{{$getRecord->purpose}}"
                        >
                    </div>
                </div>
